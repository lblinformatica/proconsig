      ...
      try {
        ...
        if (data) {
          setForm({
            codigo: data.codigo || '',
            nome: data.nome || '',
            meta: data.meta ? data.meta.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '',
            meta_diaria: data.meta_diaria ? data.meta_diaria.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '',
            ativo: data.ativo !== false
          });
        }
      } catch (err: any) {
        console.error('Erro ao buscar vendedor:', err);
        setError(err.message || 'Erro ao carregar dados do vendedor.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="animate-fade-in" style={{ maxWidth: '600px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: '2rem', gap: '1rem' }}>
        ...
        <h1 style={{ margin: 0 }}>Editar Vendedor</h1>
      </div>
